<?php
    ...
    if(!empty($_POST)) {
        ...
        $statement = $db->prepare("UPDATE produit SET Catalogue = 0 WHERE NumProd = ?"); //Prepare la modification du catalogue
            ...
            <div class="row">
                <?php 
                    ...
                    while($donnees = $reqU->fetch()){ //Tant qu'il reste des données à parcourir alors:
                        echo '<h1><strong>Supprimer le produit '.$donnees['NumProd'].' du catalogue ?</strong></h1>';
                    }
                ?>
                <br>
                <form class="form" action="supprimercatalogue.php" role="form" method="post">
                    <input type="hidden" name="id" value="<?php echo $id;?>"/>
                    <p class="alert alert-warning">Voulez-vous supprimer le produit du catalogue ?</p>
