<?php
    ...
    if(!empty($_POST)) {
        ...
        $statement = $db->prepare("UPDATE commande SET Statut = 'Payee' WHERE NumCom = ?"); //Prepare la modification du statut de l'oubli
         ...
         <div class="container admin">
            <div class="row">
                <?php 
                    ...
                    $reqU = $db->query("SELECT co.NumCom,NomCli,PrenomCli FROM commande co INNER JOIN client cl ON co.NumCli = cl.NumCli where NumCom = $id");//Selectionne tous les oublis dont l'IdOubli est égal à l'id
                    while($donnees = $reqU->fetch()){ //Tant qu'il reste des données à parcourir alors:
                        echo '<h1><strong>Le client '.$donnees['NomCli'].' '.$donnees['PrenomCli'].' a t-il payé sa commande n°'.$donnees['NumCom'].' ?</strong></h1>';
                    }
                ?>
                <br>
                <form class="form" action="accepterpaiement.php" role="form" method="post">
                    <input type="hidden" name="id" value="<?php echo $id;?>"/>
                    <p class="alert alert-warning">Voulez-vous accepter ?</p>
