...
$link = mysqli_connect('localhost','root','root','lowhifi');
...
$requete = "SELECT * FROM client WHERE NumCli=".$_GET['id']."";
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Image('Images/logotest.png',8,2);
		// Saut de ligne 20 mm
		$this->Ln(20);
...
$pdf->Cell(75,6,(utf8_decode('N° Commande : 50')),0,1,'L',1);		
$pdf->Cell(75,6,strtoupper(utf8_decode($data_voyageur['PrenomCli'].' '.$data_voyageur['NomCli'])),0,1,'L',1);	
$pdf->Cell(75,6,(utf8_decode($data_voyageur['AdrCli'].', '.$data_voyageur['CPCLi'].' '.$data_voyageur['VilleCli'])),0,1,'L',1);				
...
// $requete2 = "SELECT * FROM composer WHERE NumCom= 1";
...
// while ($data_visit = mysqli_fetch_array($result2)) {
	...
	$pdf->SetX(10);
	$pdf->MultiCell(60,8,utf8_decode('Télé TEST'),1,'C');
    // position abcisse de la colonne 2 (70 = 10 + 60)	
	$pdf->SetY($position_detail);
	$pdf->SetX(70); 
	$pdf->MultiCell(60,8,utf8_decode('10'),1,'C');
	// position abcisse de la colonne 3 (130 = 70+ 60)
	$pdf->SetY($position_detail);
	$pdf->SetX(130); 
	$pdf->MultiCell(30,8,utf8_decode('1000 €'),1,'C');
